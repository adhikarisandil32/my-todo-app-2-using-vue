...
<script setup>
    ...
    //function for button with event action removeClicked(key)
    let removeClicked = (removeFrom) => {
        props.todoLists.splice(removeFrom, 1)
    }

    let editClicked = (editFrom) => {
        const newInputText = prompt("Edit");
